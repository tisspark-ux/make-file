...
def build(wb, year: int = 2026):
    ...
    ann_colors  = ["2E5F8A", "2E5F8A", "4A7A5A", "B05050", "7A5CA8", "7A5CA8"]
    ...
    hm_colors  = ["2E5F8A", "2E5F8A"] + ["4A7A5A"] * 12 + ["B05050"]
    ...
    for i, (main_cat, sub_cat, *_) in enumerate(BUDGET_ITEMS):
        ...
        for ci, val in [(2, main_cat), (3, sub_cat)]:
            c = ws.cell(row=row, column=ci, value=val)
            c.fill = _fill("F2F2F2" if i % 2 == 0 else "FFFFFF")
            c.alignment = _align()
            c.border = _border()
    ...
    ws.conditional_formatting.add(
        ...
        ColorScaleRule(
            ...
            end_type="percentile", end_value=100,      end_color="B05050"
        )
    )
...
def _add_annual_chart(ws, header_row, data_start, data_end, anchor_row):
    ...
    bar.series[0].graphicalProperties.solidFill = "5B82BE"
